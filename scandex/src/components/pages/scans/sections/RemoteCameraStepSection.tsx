...
//  COMPONENT
// =================================
export default function RemoteCameraStepSection({
  videoRef,
  hasStream,
  onRequestCapture,
  onFocusTap,
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        onClick={handleTap}
        className={`relative aspect-[3/4] w-full overflow-hidden rounded-2xl bg-black ${hasStream ? "cursor-crosshair" : ""}`}
      >
        <video
          ref={videoRef}
          autoPlay
        ...
        {hasStream ? (
          <>
            {/* Riquadro guida per il posizionamento della carta, come sul telefono */}
            <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
              <div className="relative aspect-[63/88] h-[78%] rounded-lg shadow-[0_0_0_9999px_rgba(15,23,42,0.55)]">
            ...
            <p className="absolute bottom-3 left-0 right-0 text-center text-xs font-medium text-white drop-shadow">
              Il telefono deve posizionare la carta dentro il riquadro. Tocca
              per mettere a fuoco.
            </p>
          </>
        ) : (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/80 p-6 text-center text-sm text-white">
            ...
            In attesa dell'anteprima dal telefono. Apri "Scansiona carta" sul
            telefono collegato.
          </div>
        )}
      </div>
